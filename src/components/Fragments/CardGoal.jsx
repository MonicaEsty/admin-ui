...
function CardGoal(props) {
  ...
  const targetAmount = Number(
    safeData.target_amount ?? safeData.targetAmount ?? 0
  );

  const presentAmount = Number(
    safeData.present_amount ?? safeData.presentAmount ?? 0
  );
      ...
      desc={
        Object.keys(safeData).length === 0 ? (
          <div className="flex flex-col justify-center items-center h-full text-primary">
            ...
            <p className="text-sm font-semibold mt-3">Loading Data</p>
          </div>
        ) : (
          chartData
        )
